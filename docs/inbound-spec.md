...
commands). Builds on the `impyard relay` hand-off and the governed action path.

## Goal (concrete)

Server admins add the imp to Discord channels and administer it; it discovers
...
- **Host operator** — whoever runs `serve`/`supervise`/`listen` and holds the
  shell. Ultimate authority; owns the vault, the bot, the imps. (The `impyard`
  CLI is theirs.)
- **Server admin** — a Discord user with admin permission in the imp's server.
  Full imp administration, *including* editing **identity**, marking channels
  trusted/untrusted, and which channels the imp is in.
...
- **Untrusted participant** — a non-admin in an **untrusted** channel. Can **talk**
  to the imp (content) and nothing more — no gate approval, no admin.
...
- **Messages** (channel or DM) are **content, never commands** (D12). No matter
  ...
  governance layer; it directs the imp's attention and behavior, and every
  action the imp takes is still gated. An injection in a message can't escalate.
- **Slash commands** are **authenticated, role-checked admin**. Discord verifies
  ...
  not chat.
- **Gates** hold the *imp's* proposed actions until a human of the right role
  approves. (A human editing purpose/identity via slash command is direct,
  authenticated admin — not a gate. Gates are for the imp's own proposals, D10.)

## Channel discovery

A server admin adds the imp to a channel by granting the bot access in Discord
(channel permissions / role). The imp **discovers the channels it can see** on
...
One imp watches many channels; each has many participants. Every inbound
message the imp sees carries: channel id + name, the author's id + display name
**and resolved role** (admin / trusted / untrusted), whether it's a DM, timestamp,
text, and attachment refs. So the imp always knows *where* it is, *who* is
speaking, and *what authority* they carry.

## Waking the imp, and the judgement to respond
...
1. **When to wake (transport, cheap):** not every message spawns a run. The
   transport wakes the imp on a trigger — a DM, an @mention, an admin/trusted
   steer, or a short debounce — and batches the recent messages into one task.
2. **Whether to respond (imp, judgement):** once running, the imp decides
   ...
   often doesn't unless it's addressed or the imp has something useful.
...
- **`identity.md`** — the imp's **fixed self**, the same across every channel.
...
- **Identity** — a **human** admin edits via `/identity`; a **imp** proposal is
  **always hard-gated** (D10, imp-wide, high blast radius).
- **Purpose** — set **conversationally**, not just by command: when a trusted
  participant describes the imp's standing role in a channel, the imp
...
The imp replies through a governed action — `discord_send(channel_id, text)`
...
- **Trusted+**: `/gates ls|show|approve|deny`, `/queue ls|add|show|requeue`,
  `/purpose set|show`.
- **Admin only**: `/identity set|show`, `/channel trust|untrust`, imp
  enable/disable per channel.

These map to the existing `impyard` subcommands. Approving a gate is the
authenticated caller (trusted+) deciding — the approval desk, role-checked, still
"no model at the edge."

## The pieces

1. **`impyard listen`** — the Discord gateway client (dials out): discovers
...
5. **Slash-command handler** — role-checked, maps to safe `impyard` subcommands.
6. **`impyard connect discord`** — bot token → vault. (Owner ids are replaced by
...
- **Identity is the most privileged edit** — server admin / host op only;
  imp-proposed identity/purpose edits stay **hard-gated** (D10).
...
2. **Gateway client + discovery + roles** — `impyard listen`: dial out, discover
...
4. **Identity/purpose split** — `identity.md` + per-channel `purpose.md`; human
   edits via slash command, imp proposals hard-gated.
...
- **One primary server per imp** for identity authority; an imp spanning
...
- **Trusted participants cannot edit identity** — imp-wide, high blast radius;
