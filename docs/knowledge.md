...
What an imp learns about the world lives in a private **git repository** —
one per imp, owned by the trusted side. The imp writes notes; the host
validates them and makes the commit. History cannot be rewritten from
inside a box, and `git log` is the audit trail of how the imp's knowledge
grew.
...
`impyard imp init` creates a bare repo at
`data/imps/<name>/knowledge/repo.git` with two top-level areas:
...
git clone "$(impyard imp knowledge yuko)" /tmp/yuko-knowledge
```
...
`/opt/impyard/knowledge`, plus a host-issued **record namespace** its new
filenames must carry. On a clean exit, the host validates the checkout and
integrates it as a commit on `main`; the commit message records the imp,
...
- **Append** (normal research runs): add files under `records/` only, each
  ...
  untouched.
- **Reorganization** (`imp task add --reorganize`): holds the imp's
...
in a serialized per-imp integration lane. An abnormal exit quarantines the
...
`impyard server runs show <run>` displays a run's knowledge mode, base
...
task is imp-only by construction, so it runs clean with a writable shelf.
...
behavior) are in [configuration.md](configuration.md); per-imp overlays can
