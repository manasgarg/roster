...
Impyard is a control plane: the code checkout contains **no config and no
...
$XDG_DATA_HOME/impyard/         ~/.local/share/impyard — durable; THE BACKUP SET
  ...
  imps/<name>/                  one imp's whole footprint:
...
`$IMPYARD_ROOT/{config,data,state}` — tests, scratch deployments,
side-by-side instances) → the `XDG_*_HOME` env vars → the XDG defaults.
`IMPYARD_VAULT_DIR` / `IMPYARD_CA_DIR` override their spots individually.
...
- **No compile step.** Consumers parse the TOML straight into the runtime's
  ...
  `impyard server validate` runs the same loader and prints every error.
- **The box mounts none of this.** Isolation by absence: the container sees
  ...
  extensions are baked into the impyard-box image (`[engine] dir` is an
  optional dev override that mounts a checkout over them).
- **Imp-first data**: an imp's whole footprint is one subtree under
  `data/imps/<name>/` — export is that subtree plus its spec. Runs stay
  global (`state/runs/`) because run ids are cross-imp handles; a run's
...
- Worth doing on day one: `git init ~/.config/impyard` — your governance
