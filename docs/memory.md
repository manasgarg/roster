...
Memory is what a worker has learned from its interactions with people and
...
identity or purpose. It only informs what the worker *tries*; the gateway
...
- **Worker** — applies across all the worker's conversations (a recurring lesson,
  a communication convention). Widest blast radius, so the strictest
  defaults: worker-wide writes gate for review unless policy allows them.
...
A DM recalls all three (worker + that DM's channel memory + that user's
...
(the worker concluded). Explicit beats inferred everywhere: explicit
...
Per worker, host-side, as an append-only event log
(`data/workers/<name>/memory.jsonl`). The box never writes it — memory
operations are governed actions (`remember`, `forget`, `correct`, pin,
disable…), and the trusted side derives the worker, channel, and actor from
...
recall immediately; `roster worker memory compact` physically erases dead
content when retention or privacy requires it.

```bash
roster worker memory ls yuko --scope user --scope-id discord:123
roster worker memory show yuko note_01J…
roster worker memory correct yuko note_01J… "prefers weekly, not daily"
roster worker memory rm|pin|unpin|disable|enable yuko note_01J…
```
...
- channel task or group turn → worker + that channel's memory (private user
  notes stay out of group contexts by default);
- DM turn → worker + DM channel + that user's memory.
...
```bash
roster server runs recall <run-id>
```

## Tuning

Org-wide policy lives in `org.toml [memory]` (enable, allowed kinds, note
and recall limits, retention, whether inferred/worker-wide/cross-channel
behaviors are allowed), with per-worker `[memory]` overlays. Per channel,
trusted admins tune the same dials down — never up:

```bash
roster server channel set <id> memory off
roster server channel set <id> memory-inferred review
roster server channel set <id> memory-kinds preference,decision
roster server channel set <id> memory-retention 30
```

The precedence is simple: admin limits over worker defaults over channel
