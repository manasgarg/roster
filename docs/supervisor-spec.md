...
```
impyard queue add --imp yuko "draft the weekly digest and email it to the team"
impyard supervise            # the trusted loop: dispatch → run box → collect actions
```

The box runs. The imp researches (governed `web_search`/`fetch_pages`, already
...
the owner's `email-send` rule, sees the imp isn't yet trusted for it, and
...
```
impyard gates ls                    # → gate g-8f3a  email-send  yuko  pending
impyard gates show g-8f3a           # the exact recipients, subject, and body
impyard gates approve g-8f3a        # a human decides — no model at the edge
```
...
result — to the permanent audit log. That is the entire spine: **the imp
...
ceiling timer; humans approve in minutes or hours. So the imp cannot *wait
...
deferred. If a deferred action produces something the imp must react to, the
...
## Visibility: the imp must see its own gate state

An async gate is useless to the imp if it can't later learn what became of it.
...
- **The journal is a shared, append-only timeline, and *every* actor writes to
  ...
  and the executor (finished with this result, or failed). One per-imp log,
  `journal/<imp>/events.jsonl`, is the single place the whole story lands.
- **The gate queue is visible to the imp.** At the start of every run the
  supervisor briefs the box with its imp's open gates and any resolved since
  the last run; during a run a read-only tool lets pi re-check. pi sees only its
  own imp's gates — so it never re-proposes something already pending, and a
...
executor acts only on a real gate resolved by a real human. So the imp gets
...
The gateway resolves `imp` from the **identity token** on the connection
(the un-spoofable mechanism the box already uses), never from the envelope. The
imp cannot lie about who it is or grant itself an intent.
...
3. Consult the **trust ladder** for `(imp, intent)`:
...
credentials the box never sees:
   - **API-shaped** (email, Discord, GitHub REST): the executor issues the real
     ...
     pipeline as everything else. Imps have **no grant** to these write hosts,
   ...
   - **git push** is the exception: the gateway can't govern git's wire protocol
     ...
     `imp/<name>/*` refs. Opening/merging the PR is GitHub REST, so it goes
...
The human reviews the **exact** bytes that will go out; the imp cannot alter
the payload after proposing it. Every transition is appended to the imp's
journal, so on its next run the imp sees the gate was approved at T and the
...
**5. Trust ladder.** Per `(imp, intent)`, starting at **T0: every irreversible
gated** (§3.9). Owner config promotes an intent (optionally narrowed by a payload
predicate) to `auto`:

```toml
# org.toml or imps/<name>/imp.toml
[[trust]]
intent = "email-send"
...
**6. Task queue (§3.6).** One durable per-imp queue, `queue/<imp>/<id>.json`,
...
The queue is readable by the imp so it can dedup ("don't re-propose what's
queued").

**7. Wake-ups & the supervisor loop (`impyard supervise`).** A long-running trusted
Rust process (sibling to `serve`, sharing the same on-disk state). Triggers file
tasks; they never work inline (§3.5):
   - **schedule** — `imp.toml` cron fires a proactive task;
   - **event** — an inbound relay message (Discord/email) files a task as
     *content, never a command* (D12);
   - **manual** — `impyard queue add`;
...
`imp/<name>/<task>`. `commit_and_push` / `open_pr` are gated intents; the diff
...
runs, so an imp that spans propose → wait → react needs durable state.
`journal/<imp>/events.jsonl` is the append-only record of what happened, and
**every actor appends to it** — box, gateway, approval desk, executors — making it
the imp's single source of truth for its own history *and* its gate state (see
...
gates to the owner (D10) — the imp appends notes; only a curator step promotes
them, closing the injection→self-programming hole.

## On-disk layout (all gitignored)

```
queue/<imp>/<task-id>.json     durable per-imp task queue
gates/pending/<gate-id>.json      gated actions awaiting a human
gates/resolved/<gate-id>.json     decided gates (audit)
journal/<imp>/events.jsonl     per-imp memory + gate timeline (every actor appends)
mailbox/<imp>/                 owner→imp steer messages (delivered at turn boundaries, §3.2)
...
- Imp identity comes from the connection token, never the envelope
...
- The **journal is the imp's view; `gates/` is the authoritative state**. It is
  trusted-side and box-unwritable, and enforcement never reads the journal — so a
  box cannot conjure an approval by writing one. This is what lets the imp have
...
2. **Gates store + `impyard gates ls|show|approve|deny` + the `gate` verdict**
...
4. **`impyard supervise` + built-in queue + `impyard queue add`** — dispatch loop.
