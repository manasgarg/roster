...
use crate::{action, budget, gate, ledger, memory, paths, queue};
...
fn knowledge_head(worker: &str) -> Option<String> {
    let repo = crate::knowledge::repo_path(worker).ok()?;
...
pub fn ls(json: bool) -> Result<(), BErr> {
    ...
    for name in names {
        ...
        let gates = gate::for_worker(&name).iter().filter(|g| g.state == "pending").count();
        println!(
            "{:<12}  {:<24}  {:<6}  {:<7}  {}",
            name,
            queue_line,
...
pub fn show(name: &str, json: bool) -> Result<(), BErr> {
    ...
    let knowledge = crate::knowledge::repo_path(name).ok();
    ...
    println!("worker    {name}");
    println!("spec      {}", paths::worker_dir(name).join("worker.toml").display());
    println!("identity  {}", paths::worker_dir(name).join("identity.md").display());
    let queue_line = if counts.is_empty() {
        "empty".to_string()
    } else {
        counts
    ...
    for t in &triggers {
        println!(
            ...
            crate::runlog::one_line(t.get("prompt").and_then(|v| v.as_str()).unwrap_or(""), 60)
        );
    }
