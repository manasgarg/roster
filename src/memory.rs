...
fn contextual_policy(worker: &str, context: &RunContext) -> MemoryPolicy {
    ...
    if let Some(channel) = context.channel_id.as_deref() {
        if !crate::discord::channel_memory_enabled(channel) {
            policy.enabled = false;
        }
        if let Some(limit) = crate::discord::channel_memory_recall_max_notes(channel) {
            policy.recall_max_notes = policy.recall_max_notes.min(limit);
        }
        if let Some(limit) = crate::discord::channel_memory_recall_char_budget(channel) {
            policy.recall_char_budget = policy.recall_char_budget.min(limit);
        }
        if let Some(kinds) = crate::discord::channel_memory_allowed_kinds(channel) {
            ...
            if context.trusted_participant()
                && (basis == MemoryBasis::Explicit
                    || context
                        .channel_id
                        .as_deref()
                        .map(crate::discord::channel_memory_inferred_auto)
                        .unwrap_or(false)) =>
        {
            "auto"
        }
...
fn effective_expiry(
    ...
    if *scope == MemoryScope::Channel {
        if let Some(value) = context
            .channel_id
            .as_deref()
            .and_then(crate::discord::channel_memory_retention_days)
            .and_then(retention_expiry)
            .and_then(|s| time::OffsetDateTime::parse(&s, format).ok())
        {
            candidates.push(value);
