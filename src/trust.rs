...
mod tests {
    ...
    fn internal_recipients_auto_external_gates() {
        let rs = rules();
        // all internal → auto
        assert_eq!(evaluate("org/yuko", "email-send", &json!({"to":["a@ourco.com","b@ourco.com"]}), "gate", &rs, 0, 0), "auto");
        // one external → predicate fails → default gate
        assert_eq!(evaluate("org/yuko", "email-send", &json!({"to":["a@ourco.com","x@evil.com"]}), "gate", &rs, 0, 0), "gate");
    }
    ...
    fn earned_promotes_after_threshold_and_resets_on_denial() {
        let rs = vec![TrustRule { scope: "org".into(), intent: "email-send".into(), predicate: HashMap::new(), level: "earned".into(), after: Some(3) }];
        let p = json!({"to":["x@out.com"]});
        assert_eq!(evaluate("org/yuko", "email-send", &p, "gate", &rs, 0, 0), "gate"); // none yet
        assert_eq!(evaluate("org/yuko", "email-send", &p, "gate", &rs, 2, 0), "gate"); // under threshold
        assert_eq!(evaluate("org/yuko", "email-send", &p, "gate", &rs, 3, 0), "auto"); // reached
        assert_eq!(evaluate("org/yuko", "email-send", &p, "gate", &rs, 9, 1), "gate"); // a denial revokes it
    }
}
